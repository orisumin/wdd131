...
function search(){
  console.log("button clicked");
    ...
    //filter array with whatever the user typed in
    // user input
  ...
  //sort filtered results by hike difficulty
  const final = filteredHikes.sort(sortbyDifficulty);
  display(final);
}
...
function difficultyTemplate(difficultyLVL){
    let template = ``; 
  for (let i = 1; i <= difficultyLVL; i++){
    template += `🥾`;
  }
  for (let i = 1; i <= 5-difficultyLVL; i++){
    template += `▫️`
  }
return `<p><span class="rating" role="img" aria-label="1 out of 5">${template}</span></p>`;
