const recipes = [
	...
	},
	{
		...
		image: 'imgs/escalopes-de-poulet-a-la-creme.webp',
		recipeIngredient: [
	...
	},
	{
		...
		image: 'imgs/roasted-potatoes.webp',
		recipeIngredient: [
			'3-4 Medium Potatoes',
			'1 Tbsp Olive oil',
	...
	},
	{
		...
		image: 'imgs/black-beans-and-rice.jpg',
		recipeIngredient: [
	...
	},
	{
		...
		image: 'imgs/german-gooseberry-cake.jpg',
		recipeIngredient: [
			'For the Cake Base:',
...
function handleSearch(){
    ...
    const matchingRecipe = recipes.filter(recipe =>{
        return recipe.name.toLowerCase().includes(userinput) || recipe.description.toLowerCase().includes(userinput)|| recipe.tags.join(" ").toLowerCase().includes(userinput);
    })    

    const organizedRecipe = matchingRecipe.sort(sortByName);
    displayField.innerHTML = "";
    display(matchingRecipe);
}
...
function tagGenerator(tags){
    let tagFormat = ``;
    tags.forEach(tag => {
        tagFormat += `<p class="tag">${tag}</p>`;
    });
    return tagFormat;
}

function ratingGenerator(ratingNum){
    let ratingFormat = ``;
    for(let i = 1; i<=Math.round(ratingNum); i++){
        ratingFormat += `<span aria-hidden="true" class="icon-star">⭐</span>`;
    }
    for(let i = 1; i <= 5-Math.round(ratingNum); i++){
        ratingFormat += `<span aria-hidden="true" class="icon-star-empty">☆</span>`;
    }
    return ratingFormat;
}
...
function display(recipes){
    ...
    <img src="${recipe.image}" alt="food image">
        <div class="description">
            ...
            <p class="explanation">${recipe.description}</p>
        </div>
    </section>`;
    });
